...
class LaumioConfig {
    public: // Helper classes
        ...
        template <int N>
        class ConfigString : public Config<N> {
            public:
    ...
    private:
        std::array<Param*, 11> paramList {{
            &connect_enabled, &connect_essid, &connect_password,
            &ap_enabled, &ap_essid, &ap_password,
            &apfallback_enabled,
            &mqtt_enabled, &mqtt_host, &mqtt_user, &mqtt_password,  
        }};
};
